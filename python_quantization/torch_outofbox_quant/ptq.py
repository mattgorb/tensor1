...
def test(model, device, criterion, test_loader, best_acc):
    ...
    print(f"Top Test Accuracy: {best_acc}")
    print(f"Epoch test accuracy: {acc}")
    return best_acc
...
def main():
    ...
    test_loader = torch.utils.data.DataLoader(
        ...
        batch_size=args.test_batch_size, shuffle=False,worker_init_fn=np.random.seed(0), **kwargs)
    ...
    quant_acc=test(model_static_quantized, "cpu", criterion, test_loader, 0)
    print(f"Quantized Accuracy: {quant_acc}")
    if args.save_model:
        #torch.save(model.state_dict(), "../../weights/mnist_mlp_{}.pt".format(args.model_type))
        torch.jit.save(torch.jit.script(model_static_quantized), "../../weights/mnist_mlp_{}.pt")
