...
class GetSubnetBinary(autograd.Function):
    @staticmethod
    def forward(ctx, scores, weights, k, alpha, mask_compression_factor):

        score_agg=torch.mean(scores.clone().flatten().reshape((mask_compression_factor, 
                                                               int(scores.clone().numel()/mask_compression_factor))), dim=0)
        out = score_agg.clone()
        _, idx = score_agg.flatten().sort()
        j = int((1 - k) * score_agg.numel())
        flat_out = out.flatten()
        flat_out[idx[:j]] = 0
        flat_out[idx[j:]] = 1
        num_tiles=int(torch.ceil(torch.tensor(scores.numel()/score_agg.size(0))).item())
        tiled_tensor=out.tile((num_tiles,))[:scores.numel()]
        out=tiled_tensor.reshape_as(scores)
...
class LinearSubnet(nn.Linear):
    def __init__(self,tile,user_args, *args, **kwargs):
        ...
        self.mask_compression_factor=self.args.mask_compression_factor
    ...
    def forward(self, x):
        quantnet = GetSubnetBinary.apply(self.scores, self.weight,self.prune_rate,  self.mask_compression_factor)
...
class NetSparse(nn.Module):
    def __init__(self, args):
        ...
        self.mask_compression_factor=self.args.mask_compression_factor
        print( f"model weight tile size: {self.args.weight_tile_size}")
        print( f"fc1 tile length size: {self.fc1.scores.numel()/self.args.mask_compression_factor}")
        print( f"fc2 tile length size: {self.fc2.scores.numel()/self.args.mask_compression_factor}")

        print(f"dense model paramters:  {self.fc1.weight.numel()+self.fc1.weight.numel()}")
        print(f"tiled model parameters:  {self.args.weight_tile_size+self.fc1.scores.numel()/self.args.mask_compression_factor+self.fc2.scores.numel()/self.args.mask_compression_factor}")
...
def main():
    ...
    parser.add_argument('--mask_compression_factor', type=int, default=None, help='factor of 2')
    ...
    if args.mask_compression_factor is None: 
        print('set mask_compression_factor')
        sys.exit()

    model = NetSparse(args).to(device)


    print(f"hidden size: {args.hidden_size}, weight tile size: {args.weight_tile_size},\
           prunerate: {args.prune_rate} , , mask_compression_factor: {args.mask_compression_factor} ")
    ...
    for epoch in range(1, args.epochs + 1):
        train(model, device, train_loader, optimizer, criterion, epoch, None)
